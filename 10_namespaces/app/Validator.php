...
class Validator
{
    ...
    /**
     * Validate data againts a set of rules and set errors in the $this->errors
     * array
     * @param array $data
     * @param array @rules
     * @return boolean
    ...
    /**
     * Validate the $value of $item tonsee if it is a valid email address
     *@param string $item
     *@param string $value
     *@param string $parameter
     *@return boolean
    ...
    private function validateEmail ($item, $value, $parameter)
    {
        if (!filter_var($value, FILTER_VALIDATE_EMAIL)) {
            $this->error[$item][] = 'The' . $item . 'field should be a valid email addres';
            return false;
        }
        return true;
    }
        ...
        private function validateMin ($item, $value, $parameter)
    {
        if (strlen($value) >= $parameter == false) {
            $this->errors[$item][] = 'The' . $item. 'field should have a minimmum length of' .$parameter;
            return false;
        }
        return true;
    }
